...
- The folder is **not raw content**. It is a finished _design direction_: 11 marketing posters + 21 full-page website mockups (11 pages, 3 duplicate versions, 2 sitemap/wireframe sheets). The client has already chosen the sitemap, visual style, page layouts, form fields and copy tone.
...
| Tagline               | _Vegetable Chemistry for a Cleaner Tomorrow_                                                                                                                                                                           |
...
The mockups look **AI-generated** (garbled/misspelled text such as "REQEUST A QUOTE" on the Contact page and "Neer the Hungarian border" in the Technology footer; the same tank labels in every image). They are good for _direction_, unreliable for _facts_. Each item below is a claim a B2B buyer could check.
...
| 5   | **SFME / SME "high oxidative stability"** (SME even "especially high in unsaturated fatty acids")                           | `17-13`, `17-17`                                        | Chemically doubtful: polyunsaturated (sunflower, soy) esters normally oxidise _faster_ than rapeseed esters. Have the client's chemist rewrite these two feature lists. |
| 6   | **Identical "typical properties"** tables for RME/SFME/SME (only density differs) and they mirror EN 14214 fuel limits      | `17-11`, `17-13`, `17-17`                               | Use real product data; keep "indicative" disclaimer; the site sells _industrial/technical_ grade, so avoid implying fuel-grade compliance unless true.                  |
...
| --- | ----------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 8   | Quality page lists **EN ISO 9001, EN ISO/IEC 17025, "Certified analysis", "Certified quality", "Regulatory compliance"** as if held | Confirm which certificates/accreditations exist. If none: reword to "analysed according to EN 14103 …" and drop the word _certified_. If some: show certificate PDFs + issuer + validity.                                         |
...
- **CTA wording**: 12+ variants ("Request a Quote", "Contact / RFQ", "Start Your Project", "Discuss Your Application", "Get in Touch", …). Use **two**: primary _Request a Quote_, secondary _Talk to our experts_ (contextual variants only in secondary).
...
| -------------------------- | ------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| **1 Home**                 | Explain in 10 s what BIOCETAN does; route to a service; get an RFQ | Hero (H1 _Custom Vegetable Methyl Esters_; sub-line **"Your feedstock. Your specification. Your product."**; tagline as eyebrow) → proof strip (10–60 t/day · 2 lines · 4×15 t · in-house lab — numbers only) → **3 ways to work with us** (Made-to-Order / Toll / Custom formulation) → Products teaser (4 cards) → How a project works (5 steps) → Applications teaser (6 cards) → Quality teaser + sample CoA → Location/logistics teaser + map → FAQ (6–8 Qs) → final CTA with **inline quick form** | Request a Quote                          |
| **2 Products**             | Help a buyer pick an ester and ask for a quote                     | Intro + sub-nav → **comparison table RME / SFME / SME** (density, viscosity, acid value, ester content, CFPP, best for — _real values TBC_) → one section per product: overview, 4–5 verified key facts, typical properties (indicative), best-for applications, _Request a quote for {product}_ (prefills form) → Custom esters: possible feedstocks + how a custom spec is developed → "manufactured to your spec; CoA per batch" → CTA. SFME/SME claims per §3.1-5                                    | Request a quote · Request TDS            |
| **3 Services**             | Explain the ways to work with us; reduce buyer risk                | Intro + sub-nav → **"Which model fits you?" table** (Made-to-Order · Toll · Custom development · Pilot: _you provide_ / _we provide_ / _best for_ / _typical volume_) → capabilities panel (2 lines · 4×15 t · 60 t/day · campaigns) shown once → one section per model with its own steps (Toll: confidentiality & IP; Formulation: base ester → additives → homogenisation → QC → packaging; Pilot: **verify §3.1-3**) → CTA                                                                           | Start your project                       |
...
| **6 Quality**              | Trust                                                              | Approach → analytical capabilities & **methods table** (parameter · standard · what it tells you) → lab photo → 5-step quality process → **documents** (CoA sample / SDS / TDS / regulatory, or _Request documentation_) → certifications **only if §3.2-8 is confirmed** → CTA                                                                                                                                                                                                                          | Contact quality team                     |
...
| **9 Contact**              | Generate leads                                                     | Hero → contact card → **quick form** with "Add details" expansion → _What to send for a fast quote_ checklist → map → visit card                                                                                                                                                                                                                                                                                                                                                                         | Send request                             |
...
| _Hidden_                  | source page, product prefill (`?product=rme`), UTM params, locale |                                  | Product and application sections link _Request a Quote_ → `/contact?product=rme` or `?industry=agrochemicals`                                                         |
| _Anti-spam_               | honeypot + Cloudflare Turnstile                                   |                                  |                                                                                                                                                                       |

**Behaviour**

- Client + server validation with the same schema (zod); inline errors, `aria-live`, focus moves to first error.
- On success: inline confirmation ("We reply within 1 business day" — _only if client commits to it_), form reset, analytics event; on failure: keep data, show fallback email/phone.
- Needs JavaScript to submit (Cloudflare Turnstile cannot run without it). Visitors without JS see the email and phone instead. _(Changed from the original "works with JS off".)_
...
- [ ] Single dominant CTA (_Request a Quote_) in header, hero, each section end, sticky mobile bar
- [ ] Click-to-call `tel:` and `mailto:` everywhere; optional WhatsApp (ask client)
- [ ] Trust strip: capacity, 2 lines, in-house lab, Made in Europe, (certifications _if real_)
...
- [ ] Keyword targets (EN): _vegetable methyl ester manufacturer_, _RME / SFME / SME supplier_, _FAME toll manufacturing Europe_, _custom ester formulation_, _agrochemical solvent methyl ester_; RO equivalents
...
- Consent banner (equal-weight _Accept / Reject / Choose_, nothing pre-ticked) **only if** non-essential cookies are used; recommended path is **cookieless analytics** (Plausible/Umami) → no tracking cookies, simpler compliance
- Map is **click-to-load** (no third-party requests before consent)

### 7.6 Content assets & downloads

- Downloads centre on Quality page: CoA (sample), SDS, TDS per product, regulatory docs. **Client must supply PDFs.** Until then, replace _Download_ buttons with _Request documentation_ (pre-fills form with "Documentation").
...
| **1. Foundation** ✅ _done 2026-09-21_          | Repo, CI/CD, preview deploys, tokens, fonts, layout shell, header/footer, i18n scaffold, base components                                                           | 3 d                        | Blank pages deploy to preview; header/footer match mockup at 3 breakpoints                                   |
| **2. MVP: Home + Contact** ✅ _done 2026-09-21_ | Full Home, Contact page + working RFQ form end-to-end, legal stubs, 404, favicon                                                                                   | 5 d                        | **Launchable landing page.** Test lead reaches office@biocetan.ro; Lighthouse targets met                    |
| **3. Content pages** ✅ _done 2026-09-22_       | Products (data-driven, comparison table), Services, Applications, Technology, Quality, Facility & Logistics (real map), About                                      | 4–5 d                      | All 9 pages match approved design; content from §2/§3 resolved; comparison-table values signed off by client |
...
**Phase 3 outcome:** all 7 remaining pages built with real content, EN + RO: Products (comparison table + RME/SFME/SME/Custom sections + sticky sub-nav), Services ("which model fits you?" table + 4 sections + sticky sub-nav), Applications (6 industry sections, Agrochemicals lead), Technology, Quality (methods table with real EN standards), Facility & Logistics, About. Content stays inside the guardrails from §3: no waterless/washing or continuous/batch claim, no ">98% conversion" figure, no ISO 9001/17025 certification claim, no numeric ester spec table (replaced with a qualitative comparison + "confirmed per batch via CoA" note), no team/story section on About (not supplied, §11 Q9), SFME/SME feature lists drop the disputed oxidative-stability claim.

**Real distances, not mockup guesses (§3.1-7):** Săcueni's coordinates were geocoded via OpenStreetMap Nominatim (town centre — OSM has no street-level match for the exact address) and driving distances/times to 7 reference cities were computed via the public OSRM router (`src/config/logistics.ts`, methodology documented in its header comment). These replace the mockups' implausible numbers (e.g. a border crossing the mockups placed ~90 km away is actually ~46 km via Borș) with verifiable, if approximate, figures; the page states the method and recommends confirming time-critical deliveries with the team. Facility & Logistics also gets a real click-to-load OpenStreetMap embed (Leaflet, self-hosted JS/CSS, only the map tiles are third-party and only after a click — verified: zero tile requests before the click, tiles load correctly after).

Verified:

- **Types/tests:** `npm run check` 0 errors across 55 files, all 30 backend unit tests still pass, EN/RO key parity 636/636.
- **Browser, real Pages runtime:** every new page loads (200) with real content (no stub notice); `?product=`/`?industry=` prefill confirmed on Products/Applications → Contact; the Leaflet map loads on click with no prior third-party request and renders tiles + marker correctly; scroll-spy on the Products/Services sub-nav updates the active tab; mobile (375 px) has no horizontal overflow on any new page, including the comparison and distance tables (they scroll internally); the sticky bottom Call/Email/Quote bar and hamburger menu still work on the new pages.
- **Lighthouse (production config, local server):** Performance / Accessibility / Best practices / SEO = 100 / 100 / 100 / 100 on Products (mobile), Services (mobile), Facility & Logistics (mobile and desktop, with the unloaded map on the page).
- **Launch gate still correct:** `npm run build:prod` with `PUBLIC_ALLOW_INDEXING=true` still fails on exactly the 12 pre-existing legal-page `[TODO]` markers from Phase 2 — Phase 3 introduced none.

Deviations from this plan:

- The Products comparison table is **qualitative** (feedstock, typical use, packaging), not the numeric density/viscosity/acid-value table sketched in §4.4 — the client mockups' numeric tables were flagged as suspect (§3.1-6, identical across all three esters) and no real values have been supplied yet (§11 Q11). Numeric specs are deferred to a per-batch Certificate of Analysis / on-request Technical Data Sheet.
- Facility & Logistics' distance table uses **computed** OSRM figures, not client-supplied ones (§11 Q11 remains unanswered) — see above.
- About has no team/story section (not supplied, §11 Q9); values trimmed from 5 to 4, not 3–4 as a range (Sustainability, Quality, Partnership, European reach).
- Pilot & R&D describes pilot batches generically rather than citing the mockup's "10–2,000 L reactors" (§3.1-3, unverified — the plan's own suggested fallback).
- Technology page omits the waterless/washing and continuous/batch wording entirely (§3.1-1, -2) rather than picking a side, and omits the >98% conversion figure (§3.1-4).
...
7. **Domain**: confirmed — BIOCETAN owns biocetan.ro. DNS-over-HTTPS checks (2026-09-24, `dns.google`) show it delegated to `ns3.romarg.com` (a Romanian host), but those nameservers currently **refuse every query** ("lame delegation") — the domain resolves to nothing right now. **Update (2026-09-24, same day): owner now has access to the romarg.com account.** Still needs to be used to fix the lame delegation and, later, to point the domain at Cloudflare Pages before launch (Phase 5) — not done yet, just unblocked.
...
15. **Product-range flyers (2026-09-24)**: the boss sent 6 marketing posters naming actual formulated products — BIOCETAN Agro Carrier, MWF-E, Road Clean, Asphalt Release, Form Release, and Process Solvent (the last named only on the range-overview poster) — each with a short overview, benefits, typical applications and packaging (200 L drum / 1000 L IBC). This is the first time named, ready-made SKUs (distinct from the RME/SFME/SME base esters and the custom-formulation/toll/pilot services) showed up in client material. Integrated the same day: a new "Formulated Products" section on the Products page (one card per product, benefits and packaging taken directly from the flyers), a new "Construction & Infrastructure" industry on the Applications page (Asphalt Release + Form Release don't fit any of the 8 existing industries), and all 6 products + the new industry added to the RFQ form's dropdowns. No numeric specs were invented — only what the flyers state qualitatively.

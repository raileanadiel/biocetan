...
- `PRODUCT_VALUES` / `INDUSTRY_VALUES` (`src/lib/contact/constants.ts`) drive the RFQ form's dropdowns, the Products page's "Formulated Products" cards, and the Applications page's industry sections — all three read from the same arrays, so adding a product or industry is mechanical (extend the array + add `form.options.*` / `products.formulated.*` / `applications.*` i18n keys in both locales, nothing else to wire up).
